...
- **대쉬 속도**: 기본 속도 × 15
- **대쉬 데미지**: 40 (고정값)
- **히트박스**: 일반 충돌 판정과 동일 (히트박스 기반)
- **대쉬 간 대기**: 0.2초
- **이미지**: `michael_dash.png` 사용
- **충돌 판정**: 적 Power와 동일한 방식 (히트박스 충돌)
...
- **데미지**: 60 (기본 데미지 × 2)
- **데미지 적용 시점**: 착지 시점에만 한 번만 적용 (낙하 후 범위 내 진입해도 데미지 없음)
...
- **스폰 위치**: 플레이어 주변 랜덤 위치 (최소 거리 600px, 반경 900px)
- **등장 마커**: 등장 전 `<!>` 표시 (크기 40px, 외곽선 3px)
- **스폰 간격**: 등장 표시 후 1초 뒤 실제 등장
- **적 타입별 스폰 주기** (천국 배경만):
  - **말라크**: 5초마다 (4 + 웨이브)마리
  - **파워**: 5초마다 (0 + 웨이브)마리
  - **도미니온**: 10초마다 (1 + 웨이브)마리
